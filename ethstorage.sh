...
echo -e "\033[1;34mCreating folder 'dropxtor'...\033[0m"
echo
mkdir -p $WORKDIR/dropxtor && cd $WORKDIR/dropxtor || { echo "Failed to create or navigate to 'dropxtor' folder"; exit 1; }
echo
...
echo -e "\033[1;34mFolder 'dropxtor' and file 'app.html' created successfully.\033[0m"
echo
...
echo -e "\033[1;34mUploading 'dropxtor' folder with ethfs-cli...\033[0m"
echo
ethfs-cli upload -f "$WORKDIR/dropxtor" -a "$FLAT_DIR_ADDRESS" -c 11155111 -p "$PRIVATE_KEY" -t 1 || { echo "Failed to upload folder with ethfs-cli"; exit 1; }
echo
...
echo
eth-blob-uploader -r http://88.99.30.186:8545 -p "$PRIVATE_KEY" -f "$WORKDIR/dropxtor/app.html" -t "$EVM_WALLET_ADDRESS" || { echo "Failed to upload app.html with eth-blob-uploader"; exit 1; }
echo
...
echo -e "\033[1;34mUploading 'dropxtor' folder again with ethfs-cli...\033[0m"
echo
...
echo
ethfs-cli upload -f "$WORKDIR/dropxtor" -a "$FLAT_DIR_ADDRESS2" -c 11155111 -p "$PRIVATE_KEY" -t 2 || { echo "Failed to upload folder with ethfs-cli"; exit 1; }
echo
